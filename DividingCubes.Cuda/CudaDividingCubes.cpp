#include "pch.h"
/*#include "CudaDividingCubes.hpp"
#include <Macros.hpp>


using namespace Impacts;
using namespace Impacts::Geometry;
using namespace Impacts::Cuda;
using namespace Impacts::Cuda::Kernels;
using namespace thrust;
using namespace std;


CudaDividingCubes::CudaDividingCubes(shared_ptr<Impacts::Data::ImageDataSet> const & image, float isoValue, float subDistance = 0.1f)
{
...
void CudaDividingCubes::RunKernels()
{
	auto d_dataset = make_shared<device_vector<float>>(m_imgData->GetVector());
	auto datasetMgr = make_shared<KernelDataManager<float>>();
	...
	auto classifyCubesKernel = make_shared<ClassifyCubesKernel>(d_dataset->size(), m_dim, m_isoValue, grid3Dim);
	...
	auto compactCubesKernel = make_shared<CompactCubesKernel<unsigned>>(cubesSize);
	...
	host_vector<int> h_compactedSize = *cubeSizeOut->ReadToDevice(cubeSizeId, GPU);
	...
	auto classifySubCubesKernel = make_shared<ClassifySubCubesKernel>(d_dataset, d_dataset->size(), m_dim, m_isoValue,
	...
	KernelManager<unsigned __int64, Vertex> generatePointsKernelMgr;
	generatePointsKernelMgr.Setup(subCubesSize, subCubesId, subCubesOut, true);

	auto points = make_shared<Points>(subCubesSize);
	auto generatePointsKernel = make_shared<GeneratePointsKernel>(d_dataset, d_dataset->size(), m_dim, m_isoValue,
		grid3Dim, m_voxSize, m_subDistance, m_subDim, m_subVoxSize);
	generatePointsKernelMgr.Run(generatePointsKernel, points.get());

}

int3 CudaDividingCubes::CalculateSubDim()
{
...
float3 CudaDividingCubes::CalculateSubSize()
{
